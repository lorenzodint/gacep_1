...
class Router:
    ...
    def __init__(self):
        ...
        user = SessionManager.get(SessionKeys.key_utente_loggato())
        if user:
        ...
        if SessionKeys.key_current_page() not in st.session_state:
            SessionManager.set(SessionKeys.key_current_page(), Pagine.HOME)

    def get_current_page(self) -> Pagine:
        return SessionManager.get(SessionKeys.key_current_page())

    def set_current_page(self, page: Pagine):
        SessionManager.set(SessionKeys.key_current_page(), page)
    ...
    def render_navigation(self):
        ...
        for page in page_options:
            ...
            if st.sidebar.button(
                    page.value.capitalize(), key=f"nav_button_{page.value}"
            ):
